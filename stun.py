...
__version__ = '1.0.0'
LOG_FORMAT = "[%(levelname)s][%(asctime)s] %(message)s"    # 日志格式化输出
DATE_FORMAT = "%m/%d/%Y %H:%M:%S"                        # 日期格式
...
def _initialize():
    items = dictAttrToVal.keys()
    for i in items:
        dictValToAttr.update({dictAttrToVal[i]: i})
    items = dictMsgTypeToVal.keys()
    for i in items:
        dictValToMsgType.update({dictMsgTypeToVal[i]: i})
...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
    ...
    data = binascii.unhexlify(str_data)
    recvCorr = False
    while not recvCorr:
        recieved = False
        count = 3
        while not recieved:
            ...
            try:
                ...
                if count > 0:
                    ...
                    retVal['Resp'] = False
                    return retVal
        msgtype = (binascii.hexlify(buf[0:2])).decode()
        bind_resp_msg = dictValToMsgType[msgtype] == "BindResponseMsg"
        rb = binascii.hexlify(buf[4:20]).decode().upper()
        if bind_resp_msg and tranid.upper() == rb:
            recvCorr = True
            retVal['Resp'] = True
            len_message = int(binascii.hexlify(buf[2:4]), 16)
            len_remain = len_message
            base = 20
            while len_remain:
                attr_type = binascii.hexlify(buf[base:(base + 2)]).decode()
                attr_len = int(binascii.hexlify(buf[(base + 2):(base + 4)]), 16)
                if attr_type == MappedAddress:
                    port = int(binascii.hexlify(buf[base + 6:base + 8]), 16)
                    ip = ".".join([
                        str(int(binascii.hexlify(buf[base + 8:base + 9]), 16)),
                        str(int(binascii.hexlify(buf[base + 9:base + 10]), 16)),
                        str(int(binascii.hexlify(buf[base + 10:base + 11]), 16)),
                        str(int(binascii.hexlify(buf[base + 11:base + 12]), 16))
                    ])
